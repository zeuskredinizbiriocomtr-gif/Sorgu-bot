...
# --- WEB ARAYÜZÜ (Sitede Verileri Gösterir) ---
...
async def index():
    try:
        ...
        return "<h3>Henüz veri yüklenmedi veya hata oluştu.</h3>"
...
def akilli_ayikla(metin):
    sonuclar = {}
    # Süslü blokları veya düz satırları T.C. üzerinden yakalar
...
async def dosya_isle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Önce JSON olarak deniyoruz, olmazsa akıllı ayıklayıcıya geçiyoruz
        try:
            temiz_veri = json.loads(metin)
            if isinstance(temiz_veri, dict) and "TCKN" in temiz_veri:
                temiz_veri = {temiz_veri["TCKN"]: temiz_veri}
        except:
            temiz_veri = akilli_ayikla(metin)
        
        if temiz_veri:
            veriyi_kaydet(temiz_veri)
            await status.edit_text(f"✅ {len(temiz_veri)} Kayıt Eklendi!\n🌐 Site: {BASE_URL}")
        else:
            await status.edit_text("❌ Geçerli veri bulunamadı.")
    except Exception as e:
        await status.edit_text(f"❌ Hata: {str(e)}")

async def main():
    # API ve Botu aynı anda başlatır
